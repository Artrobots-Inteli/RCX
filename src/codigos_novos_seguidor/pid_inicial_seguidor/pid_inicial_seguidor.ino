...
bool cruzouLinha = false;  // Para verificar se o sensor já cruzou a linha
...
void loop() {
  ...
  if (running) {
    ...
    bool estadoSensor = digitalRead(13);

    // Verifica se o sensor entrou na linha branca (transição de HIGH para LOW)
    if (estadoSensor == LOW && !cruzouLinha) {
      cruzouLinha = true;  // Marca que o sensor cruzou a linha
    }

    // Verifica se o sensor saiu da linha branca (transição de LOW para HIGH)
    if (estadoSensor == HIGH && cruzouLinha) {
      contadorLinhasBrancas++;  // Incrementa o contador ao sair da linha
      cruzouLinha = false;  // Reseta para detectar a próxima passagem
